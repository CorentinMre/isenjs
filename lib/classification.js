...
class Event {
    constructor(id, start, end, class_name, type, subject, description, instructors, start_time, end_time, room, class_info) {
...
class PlanningReport {
    ...
    toString() {
        ...
        return `PlanningReport(data=${JSON.stringify(this.data)}, infoSalles=${JSON.stringify(this.infoSalles)})`;
    }
...
class SchoolReport {
     ...
     */
    toString() {
        return `SchoolReport(nbReports=${this.nbReports}, data=${JSON.stringify(this.data)})`;
    }
    ...
    get(key) {
        if (key === 'nbReports') {
            ...
            throw new Error(`Invalid key: ${key}, valid keys are 'nbReports' and 'data'`);
        }
    }
