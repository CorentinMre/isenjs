...
class Client {
    ...
    logout() {
        ...
        this.session.get(
            "https://auth.isen-ouest.fr/cas/logout"
        );
    }
    ...
    async classMember(cycle, annee, ville, exactClass = null) {
        try {
            ...
            try{
                if (annee >= 3 ){
                    if (this.capitalizeFirstLetter(ville) == "Brest"){
                        choix_groupe = `A${annee}${this.capitalizeFirstLetter(ville)[0]} Groupe ${cycle.toUpperCase()} ${this.annee}`;
                    }else if (this.capitalizeFirstLetter(ville) == "Rennes" && annee == "2"){
                        choix_groupe = `${cycle.toUpperCase()}${annee} ${this.capitalizeFirstLetter(ville)} ${this.annee}`;
                    }
                    else{
                        choix_groupe = `A${annee}${this.capitalizeFirstLetter(ville)[0]} ${cycle.toUpperCase()}${annee} ${this.annee}`;
                    }
                }else{
                    choix_groupe = `${cycle.toUpperCase()}${annee} ${this.capitalizeFirstLetter(ville)} ${this.annee}`;
                }
            ...
            // Check if the class exists
            this.__checkClassExist(cycle.toUpperCase(), annee, this.capitalizeFirstLetter(ville));
