...
class Moodle {
  ...
  constructor(session, cookies) {
    // Initialization of instance variables
    this.ses = session;
    this.cook = this.getEssencialCookie(cookies);
  ...
  async init() {
    ...
    // let importantCookie = this.getCookie("MoodleSession");
    // this.cook = [importantCookie];
    // this.updateSession();
    this.removeCookiesDuplicate();
  }
  ...
  getEssencialCookie(cookies) {
    let essencial = [];
    for (let i = 0; i < cookies.length; i++) {
      if (cookies[i].startsWith("JSESSIONID")) {
        essencial.push(cookies[i]);
      }
      else if (cookies[i].startsWith("AUTH_SESSION_ID")) {
        essencial.push(cookies[i]);
      }
      else if (cookies[i].startsWith("KC_RESTART")) {
        essencial.push(cookies[i]);
      }
      else if (cookies[i].startsWith("AUTH_SESSION_ID_LEGACY")) {
        essencial.push(cookies[i]);
      }
      else if (cookies[i].startsWith("KEYCLOAK_LOCALE")) {
        essencial.push(cookies[i]);
      }
      else if (cookies[i].startsWith("KEYCLOAK_IDENTITY")) {
        essencial.push(cookies[i]);
      }
    }
    return essencial;
