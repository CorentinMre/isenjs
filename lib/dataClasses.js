...
class WebAurion {
    ...
    constructor(session, cookies) {
        ...
        this.annee = getActualScholarPeriod();
    }
    ...
    async init() {
        ...
        } while ((response.status == 301 || response.status == 302) && redirCount++ < 20);
     ...
     */
    async __getWorkingTime(req, start_date = null, end_date = null, isOtherPlanning = false, infoSalles = null) {
        try {
    ...
    async roomPlanning(city, start_date = null, end_date = null) {
        ...
        listOfClasses.each((index, element) => {
            ...
            if (classId === "ui-menuitem") {
                ...
                let payload = soup(element).find("a.null").attr("onclick").split("'form',")[1].split(").submit")[0].replace(/'/g, '"');
        ...
        try{
            ...
            throw new Error("Enter good information please ! Only Caen, Rennes, Nantes for the moment");
        }


        const response = await this.session.post(this.baseMainPageUrl, classPlanning[city].payload);
        ...
        const lastPayload = {
            "form:j_idt193_reflowDD": "0_0",
            "form:j_idt193:j_idt198:filter": "",
            "form:j_idt193_checkbox": Object.keys(classRoom).map((key, index) => ({ [index]: "on" })),
            "form:j_idt193_selection": Object.values(classRoom).map(salle => salle.id).join(','),
            "form:j_idt261": "",
            "form:j_idt271_input": "275805"    
        };
        ...
        let planningPage = await this.session.post("https://web.isen-ouest.fr/webAurion/faces/ChoixPlanning.xhtml", payload);

        return this.__getWorkingTime(planningPage, start_date, end_date, true, classRoom);
    }
    ...
    async getAllUserInformations(){
        ...
        classLigne.each((index, element) => {
            ...
            const key = soup2(element).find("div.colonne1 span").text().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
...
class Moodle {
    ...
    async init() {
        const baseReq = await this.session.get(this.baseUrl, {
            maxRedirects: 0,
                validateStatus: function () {
                    return true;
                }});
        ...
        } while ((response.status == 302 || response.status == 301) && redirCount++ < 20);
    }
    ...
    getCookie(cookieName) {
        for (let i = 0; i < this.cookies.length; i++) {
            if (this.cookies[i].startsWith(cookieName)) {
